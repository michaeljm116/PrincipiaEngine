...
public:
	Quadtree() {};
	Quadtree(AABB region, std::vector<T*> ents) {
		m_region = region;
		m_objects = ents;
	...
	AABB m_region;						//Bounding Box of the Quadtree
	std::vector<T*> m_objects;			 //List of entities it holds


	Quadtree* m_ChildNode[4];				//Possible children for the node
	...
	void Insert(T* ent, Quadtree* par) {
	...
	void Update(T* ent, Quadtree* par) {
		// Find which child object to update
	}

	Quadtree* CreateNode(AABB region, std::vector<T*> ents, Quadtree* par) {
		Quadtree<T>* octo = new Quadtree(region, ents);
		octo->parent = par;
		return octo;
	}
	Quadtree* CreateNode(AABB region, std::vector<T*> ents) {
		return CreateNode(region, ents, this);
	}
	Quadtree* CreateNode(AABB region, T* ent, Quadtree* par) {
		std::vector<T*> ents;
		ents.push_back(ent);
		return CreateNode(region, ents, par);
	}
	Quadtree* CreateNode(AABB region, T* ent) {
		return CreateNode(region, ent, this);
	}

	//Cencter Check is based off the idea that the Quadtree is organized as :
